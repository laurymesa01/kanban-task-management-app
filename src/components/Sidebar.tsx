...
const Sidebar = ({ isOpen, onHide }: { isOpen: boolean; onHide: () => void }) => {
  const { state, dispatch } = useKanban();
  const { boards, activeBoardIndex } = state;

  return (
    <aside className={`flex flex-col shrink-0 h-full pb-8 border-r border-lines-light dark:border-lines-dark overflow-hidden transition-all duration-300 ${isOpen ? 'w-70' : 'w-0'}`}>
        <section className="h-full flex flex-col justify-between mt-8">
            <div className="pr-4">
                ...
                <div className="flex items-center gap-2 px-6 py-2 mt-4">
                    ...
                    <button className="flex items-center gap-1 text-main-purple heading-m cursor-pointer" onClick={() => dispatch({ type: 'TOGGLE_NEW_BOARD_PANEL'})}>
            ...
            <div className="px-4">
                <div className="w-full flex items-center gap-2 justify-center bg-light-grey rounded-md py-3">
                    ...
                    <button
                        className="w-10 h-5 bg-main-purple hover:bg-main-purple-hover rounded-full flex items-center px-1 cursor-pointer transition-colors"
                ...
                <button onClick={onHide} className="mt-6 heading-m text-medium-grey flex items-center gap-1 cursor-pointer">
